...
function extractCurrentBattlesData(html) {
  ...
  for (let fight_no = 0; fight_no < num_fights; fight_no++) {
    ...
    for (let side = 0; side < 2; side++) {
      ...
      let score;
      try {
        /* This fails during te very first hour */
        score = all_scores[offset]["children"][0]["data"];
      } catch(err) {
        score = '0';
      }
      hero_data["name"] = name;
...
function extractAllHeroesNames(html) {
  ...
  if (!util.getEventStarted()) {
    ...
    const pre_fetch_heroes = (config.PRE_FETCH_HEROES === 'true') || (time_till_event_starts < ms_in_fourteen_hours);
    if (pre_fetch_heroes) {
      ...
      for (let i = 0; i < all_names.length; i++) {
        ...
        heroes_names.push(util.getCleanName(hero_name));
      }
  ...
  if (round_no === 1) {
    console.log("Round 1. Getting data from the first <article>")
    ...
    for (let i = 0; i < name_tags.length; i++) {
      heroes_names.push(util.getCleanName(name_tags[i]["children"][0]["data"]));
    }
    return heroes_names;
  }
