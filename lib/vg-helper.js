...
function getMultiplierAndMinutesLeft(round_number) {
  ...
  const multiplier = ((rounded_hours * 0.1) + 3.1).toFixed(1);
...
function createCurrentBattlesMessages(data, round_number, options) {
  ...
  const ed_multiplier = ((multiplier - 1) / 2).toFixed(2);
  ...
  for (let i in data) {
    ...
    const advantage = (heroes[winner]['score'] / heroes[loser]['score']).toFixed(4);
    const multiplier_active = (advantage > 1.01);

    if (multiplier_active) {
      let loser_id = heroes[loser]['name'];
      let winner_id = heroes[winner]['name'];
      /* Find roles if possible. Fallback to role names */
      if (options.mention_army && GUILD_CHANNEL) {
        try {
          /* Find loser role */
          if (!options.winner_only) {
            const loser_army_role_name = util.getRoleNameFromHeroName(heroes[loser]['name']);
            const loser_army_role = GUILD_CHANNEL.guild.roles.find(item => item.name === loser_army_role_name);
            if (loser_army_role) {
          ...
          if (!options.loser_only) {
            const winner_army_role_name = util.getRoleNameFromHeroName(heroes[winner]['name']);
            const winner_army_role = GUILD_CHANNEL.guild.roles.find(item => item.name === winner_army_role_name);
            if (winner_army_role) {
      ...
      if (options.loser_only) {
        ...
        messages.push(
          `x${ed_multiplier} multiplier for ${winner_id} for next ${minutes_left} minutes against ${heroes[loser]['name']}`
          );
      } else {
        let msg = `x${multiplier} multiplier for ${heroes[loser]['name']}`;
        msg += ` against ${heroes[winner]['name']} with x${ed_multiplier} multiplier for next ${minutes_left} minutes`;
        messages.push(msg);
      }
