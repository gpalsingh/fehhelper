...
function createCurrentBattlesMessages(data, options) {
  let messages = [];
  ...
  for (let i in data) {
    ...
    messages.push(`${heroes[winner]['name']} beating ${heroes[loser]['name']} by ${difference}`);
  }
...
function getCurrentBattlesMessages(options) {
  return scraper.getCurrentBattlesData()
  .then(data => {
    return createCurrentBattlesMessages(data, options);
  })
  .catch(err => {
    console.log(err);
...
function isArmyWinning(army_score, enemy_score) {
  advantage_factor = (army_score / enemy_score).toPrecision(4);
  return advantage_factor > 1.01;
}
...
module.exports = {
  getGauntletStatusMessage: function() {
    ...
    if (battles_on) return getCurrentBattlesMessages({single: true});
