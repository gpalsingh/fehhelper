...
function createArmyRolesAllHereos(client) {
  scraper.getAllHeroesNames().then(async all_heroes_names => {
    ...
    role_names = all_heroes_names.map(name => config.ARMY_ROLE.replace(/#/, name));
...
function setupArmyRolesAllHeroes(client) {
  /* Retry at start of every hour */
  const offset =  util.getMsTillNextHour() + 5 * 60 * 10000;
  ALL_HEROES_ROLES_TIMER = setInterval(createArmyRolesAllHereos, offset, client);
