...
function sendAnnouncement(announce_channel, msg) {
  if (TESTING) {
    console.log(msg);
    return;
  }
...
function ucfirst(s) {
  const converted_str = s.toLowerCase();
  return converted_str.charAt(0).toUpperCase() + converted_str.slice(1);
}

function getCleanName(hero_name_raw) {
  return ucfirst(latinize(hero_name_raw).replace("'", ""));
}
